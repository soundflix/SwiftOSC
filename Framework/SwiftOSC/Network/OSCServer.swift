...
//            }
        ...
        listener?.stateUpdateHandler = { [weak self] (newState) in
            ...
            switch newState {
            case .ready:
                os_log("Server '%{Public}@': Ready, listening on port %{Public}@, delegate: %{Public}@, indicatesPort: %{Public}@", log: SwiftOSCLog, type: .default, self.name ?? "<noName>", String(describing: self.listener?.port ?? 0), String(describing: self.delegate), self.indicatesPort)
